...
class Data extends Dbh
{
    ...
    {
        $sql = "SELECT innameapparaat.ID, innameapparaat.InnameID, innameapparaat.ApparaatID, innameapparaat.Ontleed, Innames.MedewerkerID, innames.Tijdstip, apparaten.Naam AS Anaam FROM innameapparaat INNER JOIN innames ON innameapparaat.InnameID=innames.ID
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo '<td>' . "<a href='../classes/delete.class.php?innameapparaatID=$row[ID]&apparaatID=$row[ApparaatID]'><button type='button' class='btn btn-dark'>delete</button></a>" . '</td>';
            echo "</tr>";
        }
    }
